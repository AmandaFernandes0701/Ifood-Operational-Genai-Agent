...
export const AnalysisWidget: React.FC<AnalysisWidgetProps> = ({ data }) => {
  ...
  const isHighConfidence = data.confidence === 'Alto';
        ...
        <div className="relative flex items-center justify-center w-6 h-6 rounded-full bg-red-100 dark:bg-red-900/40 text-[#EA1D2C] dark:text-red-400">
          ...
          {!hasViewed && (
            <span className="absolute -top-1 -right-1 flex h-2.5 w-2.5">
              {/* Pulse animation ONLY if NOT high confidence (Attention Required) */}
              {!isHighConfidence && (
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
              )}
              {/* Green for High Confidence, Red for Low/Medium */}
              <span className={`relative inline-flex rounded-full h-2.5 w-2.5 border-2 border-white dark:border-gray-800 ${isHighConfidence ? 'bg-green-500' : 'bg-[#EA1D2C]'}`}></span>
            </span>
          )}
        </div>
